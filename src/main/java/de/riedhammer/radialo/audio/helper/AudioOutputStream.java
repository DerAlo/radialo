...
/**
 * AudioOutputStream is equivalent to javax.sound.sampled.AudioInputStream for
 * audio playout. <p/> AudioOutputStream also provides audio codec conversion,
 * as javax.sound.sampled.AudioSystem does for the corresponding
...
public class AudioOutputStream extends OutputStream {
    ...
    /**
     * Creates a new AudioOutputStream from a SourceDataLine converting the
     * audio format.
     */
    public AudioOutputStream(SourceDataLine source_line, AudioFormat format)
            throws IOException {
        this.source_line = source_line;
     ...
     * with this stream.
     */
    public void close() { // source_line.close();
    }
    ...
    public void write(byte[] b) throws IOException {
        ...
        if (!source_line.isRunning()) source_line.start();
        if (output_stream!=null) { output_stream.write(b); int
                len=input_stream.read(buff,0,buff.length);
            source_line.write(buff,0,len); } else {
